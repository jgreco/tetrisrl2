...
int al_push(void *data, arraylist L)
{
	al_insert(data, al_first(L), L);

	return 0;
}

void *al_pop(arraylist L)
{
	if(al_empty(L))
		assert(0);

	void *result = al_retrieve(al_first(L), L);
	al_delete(al_first(L), L);
...
int resize(arraylist L)
{
	...
	void *tmp = realloc(L->data, ((int)(i*2))*sizeof(void*));
